...
- "contradicciones": cuando lo DECLARADO en la hoja de términos no coincide con lo que dice el texto. "campo" es el nombre técnico del campo; "declarado" repite en palabras lo que se declaró; "valor_segun_texto" es la clave de opción de ese campo (de la lista entre corchetes) que describe lo que el TEXTO realmente dice, o cadena vacía si ninguna encaja.
...
export function armarPrompt(terminos, texto) {
  const hoja = Object.keys(CAMPOS).map(k => `- ${k} (${CAMPOS[k].label}) [opciones: ${Object.keys(CAMPOS[k].opciones).join(', ')}]: ${declaradoLegible(k, terminos[k])}`).join('\n')
...
const CONTRA = [
  { campo: 'datosPagador', valor: 'nombre_contacto', aplica: t => !(t.datosPagador || []).some(d => PERSONALES.includes(d)), re: /listado de pacientes|nombre, tel[eé]fono|estado de seguimiento|resultado de los estudios/i, explicacion: 'El texto entrega datos de cada paciente al pagador.' },
  { campo: 'dirigeRuta', valor: 'sugiere', aplica: t => t.dirigeRuta === 'no', re: /preferentemente|se sugerir[aá] el de|únicamente en sus/i, explicacion: 'El texto inclina la ruta hacia el pagador.' },
  { campo: 'familia', valor: 'automatica', aplica: t => t.familia !== 'automatica', re: /remitente recibir[aá]|suscriptor recibir[aá]|recordatorios a su hermana/i, explicacion: 'El texto manda información a un familiar sin decisión de la paciente.' },
  { campo: 'basePago', valor: 'comision_medicamento', aplica: t => !['comision_medicamento', 'comision_prueba'].includes(t.basePago), re: /comisi[oó]n|\d+ ?% del valor/i, explicacion: 'El texto liga el pago a ventas.' },
  { campo: 'consentimiento', valor: 'aviso_general', aplica: t => t.consentimiento === 'expreso', re: /implica la aceptaci[oó]n|aceptando el aviso/i, explicacion: 'El texto usa consentimiento tácito.' },
]
...
export function leerSimulado(texto, terminos) {
  ...
  for (const c of CONTRA) {
    ...
    if (o) contradicciones.push({ campo: c.campo, declarado: declaradoLegible(c.campo, terminos[c.campo]), cita: o, explicacion: c.explicacion, valor_segun_texto: c.valor })
  }
...
export function verificarCitas(crudo, texto) {
  ...
  for (const c of (crudo?.contradicciones || []).slice(0, 5)) {
    ...
    // El modelo solo PROPONE el valor; la coordinadora lo aplica con un toque y las reglas recalculan.
    const valor = CAMPOS[c.campo].opciones[c.valor_segun_texto] ? c.valor_segun_texto : null
    contradicciones.push({ campo: c.campo, declarado: recorta(c.declarado, 300), cita: recorta(c.cita, 400), explicacion: recorta(c.explicacion, 500), valor })
  }
